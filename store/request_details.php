...
<div class="row mb-4 d-flex justify-content-center g-0">
 <div class="col-md-12 bg-white shadow-sm p-3">
     ...
     <?php 
     if($statusCheck != "pending" && $statusCheck != "approved") {
     ?>
     ...
     <div class="table-responsive">
         ...
         <?php if ($statusCheck == "approved") { ?>
             <div class="d-flex justify-content-end mt-2">
                 <?php ?>
                 <a class="btn me-2 px-3 btn-outline-success" data-bs-target="#confirm<?php echo $count ?>" data-bs-toggle="modal">ยืนยันการเบิก</a>
...
if (isset($_POST['confirm'])) {
    ...
    if ($is_stock_sufficient) {
        // ถ้าทุกชิ้นเพียงพอ ให้ปรับปรุงสต๊อกและสถานะ
        $queryDetails = mysqli_query($conn, $sqlDetails); // รันใหม่เพื่ออัปเดตสต๊อก
        while ($rowDetail = mysqli_fetch_array($queryDetails)) {
            $part_id = $rowDetail['part_id'];
            $requested_amount = $rowDetail['amount'];
        ...
        // อัปเดตสถานะคำร้องเป็น "confirmed"
        $sqlApprove = "UPDATE requests SET status = 'confirmed' WHERE id = $req_id";
        ...
        failed("อะไหล่บางชิ้นไม่เพียงพอ กรุณาตรวจสอบสต๊อก", "index.php");
    }
}
